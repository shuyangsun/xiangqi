...
//   Game game;
//   // Initialize board with one piece (red general) at (8,4); every other
//   square
//   // is empty.
//   game.ResetFromPos({{Pos(8, 4), R_GENERAL}});
//   // Query a position with no piece.
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(0, 0));
...
//   game.ResetFromPos({{Pos(8, 4), R_GENERAL}});
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(8, 4));
...
//       {Pos(9, 5), R_ADVISOR},
//   });
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(9, 4));
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(1, 4));
...
//       {Pos(0, 5), B_ADVISOR},
//   });
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(0, 4));
...
// // Place the red general at (8,4) in its palace and the black general at
// (1,4)
// // in its palace. There are no pieces between them, so the red general's
// moves
...
// // Insert a blocking piece between the generals so that the flying move is
// not
// // available.
// TEST(PossibleMovesTest, RedGeneralFlyingBlocked) {
//   Game game;
//   // Place red general at (8,4), black general at (1,4), and a blocking piece
//   at
//   // (5,4).
//   game.ResetFromPos(
//       {{Pos(8, 4), R_GENERAL}, {Pos(1, 4), B_GENERAL}, {Pos(5, 4),
//       R_SOLDIER}});
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(8, 4));
...
// // Place the black general at (1,4) and the red general at (8,4). With no
// block
...
//   Game game;
//   // Place black general at (1,4), red general at (8,4), and a blocking piece
//   at
//   // (3,4).
//   game.ResetFromPos(
//       {{Pos(1, 4), B_GENERAL}, {Pos(8, 4), R_GENERAL}, {Pos(3, 4),
//       B_SOLDIER}});
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(1, 4));
//   EXPECT_FALSE(moves[8][4]);
// }
...
// // Here, we place the red general at (8,4) and the black general at (1,5).
// Since
// // they are not aligned on the same column, the flying general move should
// not
// // be marked.
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(9, 3));
//   // Advisors move one square diagonally inside the palace.
//   // For (9,3), only (8,4) is valid (since (8,2) is outside the palace and
//   moves
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(7, 2));
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(5, 5));
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(5, 5));
//   // With the upward leg blocked, moves that depend on it ((3,4) and (3,6))
//   must
//   // not be allowed.
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(5, 5));
//   // Upward: (4,5) should be available (empty), but (3,5) should not be
//   (blocked
...
//   // enemy piece at (5,3).
//   game.ResetFromPos(
//       {{Pos(5, 5), R_CANNON}, {Pos(5, 4), R_SOLDIER}, {Pos(5, 3),
//       B_SOLDIER}});
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(5, 5));
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(6, 5));
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(4, 5));
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(3, 5));
...
//   Board<bool> moves = PossibleMoves(game.CurrentBoard(), Pos(5, 4));
